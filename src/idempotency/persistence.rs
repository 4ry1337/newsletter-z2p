...
pub async fn get_saved_response(
    ...
    if let Some(r) = saved_response {
        let staus_code = StatusCode::from_u16(r.response_status_code.try_into()?)?;
        let mut response = HttpResponse::build(staus_code);
...
pub async fn save_response(
    ...
    let response_status_code = response_head.status().as_u16() as i16;
